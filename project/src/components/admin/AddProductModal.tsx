...
interface AddProductModalProps {
  isOpen: boolean;
  onClose: () => void;
  product?: any; // If provided, modal is in edit mode
}
...
export default function AddOrEditProductModal({ isOpen, onClose, product }: AddProductModalProps) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setImageFiles([]); // Don't prefill files
    } else {
      reset();
  ...
  const onSubmit = async (data: ProductForm) => {
    ...
    try {
      // Upload images to Supabase Storage if new files selected
      ...
      // If editing, keep old images if no new ones selected
      if (product && uploadedImageUrls.length === 0) {
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {imageFiles.map((file, idx) => (
                <div key={idx} className="w-20 h-20 bg-gray-100 rounded overflow-hidden flex items-center justify-center">
                  <img
